...
### Coverage matrix — post-review correction

| # | Found | Reality | What was done |
|---|---|---|---|
| AC-7 | `TEX-023`'s reason read "defer to when we implement .Rnw Sweave manuscripts" | `.Rnw`/`.Rmd` shipped 2026-07. Nine directives (`CLS-003`, `CLS-030`, `TEX-023`, `SG-038`–`SG-043`) carried reasons written when `.Rnw` was unimplemented, and I transcribed them into the YAML without re-judging. Two were wrong about *scope*, not just wording: `SG-041` and `SG-043` are checkable now. | All nine reasons rewritten against what the engine actually does (`.Rnw` chunks become `Sinput` envelopes so `CODE-*`/`WIDTH-001` lint them; `echo=FALSE` chunks are blanked before rules run; `preamble.py` already reads `nojss`). `SG-041`/`SG-043` moved `out_of_scope` → `not_checked`, which is the honest home: they enter the denominator as gaps. Ratio **80 of 83 → 80 of 85**; counts **76/4/5/64**. Three now-unblocked rule candidates recorded in `roadmap/follow-ups.md`. |

| AC-8 | (not anticipated) | Adding `SG-041` — the first `not_checked` provision quoting `\documentclass[nojss]{jss}` — broke `terminal_parity`. Cause: `rich` parses `[word]` in a table cell as console markup and drops it, and `rust/jsslint-core/src/terminal.rs` **deliberately reproduced** that (`strip_markup`, documented as "one genuine rich quirk") so the engines would stay byte-identical. Both agreed; both were wrong. Live since 1.0 for every message or suggestion quoting a LaTeX optional argument — `\citep[e.g.][]{key}` rendered as `\citep[]{key}`, `\section[plain]{markup}` as `\section{markup}`. Item S made it far likelier by quoting source text. | Python escapes cell text before rich sees it (`rich.markup.escape` on message, suggestion, provision, skip reason, file path, baseline path); `strip_markup` and its four call sites deleted from Rust. Regression tests both sides (`TestRichMarkupIsNotInterpreted`, `terminal::markup_tests`). JSON/SARIF/HTML were never affected. Separate CHANGELOG *Fixed* entry. |

**Process note**: nothing mechanical could have caught this. The validator checks structure and rule liveness, not whether a `reason` is still true. A `reason` that names a *precondition* ("defer until X") silently rots once X ships — worth a convention that such reasons cite a spec or issue that can be closed.
