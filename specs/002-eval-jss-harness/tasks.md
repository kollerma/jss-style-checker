...
- [~] T014 [P] [US1] Grew `examples/` to **6 vignettes covering all 4 smoke-rule code paths**: 5 placeholder vignettes (`placeholder_clean`, `placeholder_cite_violation`, `placeholder_src_violation`, `placeholder_bib_violation`, `placeholder_parse_failure`) plus **`jss_template_article`** — a verbatim copy of the canonical CTAN `jss` demo article from `docs/jss-template/`. This real JSS-authored manuscript is the highest-value calibration entry: the 8 `JSS-SRC-001` violations it triggers under Step 1's default 80-col threshold are by definition false positives, which **drives the Step 3 retune** of that rule. Still deferred: 4 more real CRAN vignettes (candidates probed — most ship `.Rnw` not `.tex`; suitable plain-tex vignettes are scarce); arXiv preprints almost universally use `\documentclass{article}` rather than `jss.cls` so they don't meaningfully exercise the style rules.
...
- [~] T029 [US3] Expanded `eval/corpus-manifest.csv` to **6 `manual`-source rows** mirroring the grown corpus under `examples/`, each with a deterministic directory-content SHA256 as the integrity marker. Real `cran`-source rows with upstream tarball SHA256s remain deferred pending real CRAN vignette selection in T014 (CRAN itself IS reachable from this sandbox — verified).
