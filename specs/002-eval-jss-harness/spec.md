...
- Known AI blind spots (documented for skip-list candidates per FR-018):
  - Qwen3 has been observed mislabelling `param=value` constructs inside `\code{}` as false positives (the linter flags them legitimately).
  - **Qwen3 rubber-stamps numeric-assertion messages (e.g., `"Line length N exceeds the 80-column limit"`) as true positives** even when the underlying style decision is whether the limit itself is justified. In an end-to-end run on 2026-04-23, Qwen3-30B-A3B-GGUF scored the canonical JSS Demo article's `JSS-SRC-001` violations at 57% precision; a human reviewer scored the same violations at 11% (all FP, because the reference template is by definition compliant). Supplying the ±3-line source context to the classifier closed about half the gap; the residual bias is a candidate for the `JSS-SRC-001` skip-list entry when the rule lands in Step 3.
  The configuration mechanism that opts rules out of AI labelling (FR-018) is how such blind spots are contained — not by retraining the model, not by hard-coding exceptions.
