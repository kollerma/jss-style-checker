...
def _seed_one(cx, pid: str, examples_path: str, rationale: str, *, write: bool) -> str:
    ...
    lines.append("#")
    lines.append("# The [[violations]] below are SEEDED from precision-review")
    lines.append(f"# verdicts (eval.db, {SEED_DATE}): each is a review-confirmed")
    lines.append("# TRUE POSITIVE — VERIFY it, and add the false negatives the")
    lines.append("# linter missed (those are not in eval.db). Reviewed FALSE")
    lines.append("# POSITIVES are listed (commented) at the bottom: do NOT add")
    lines.append("# them as violations, but confirm the FP judgement as you read.")
    if SEED_EXCLUDE_RULES:
        ...
        lines.append("# `python -m eval.crossref_doi_lookup` (records the actual DOI).")
    lines.append("")
    lines.append("[meta]")
