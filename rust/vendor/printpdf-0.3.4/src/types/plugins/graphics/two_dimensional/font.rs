...
///
/// vendored-fix (see `rust/vendor/printpdf-0.3.4/NOTICE.md`): upstream wrote
/// the target codepoint as a single `{:04x}` hex value, which silently
/// truncates/misrenders anything above U+FFFF (a raw 5+-hex-digit value is
/// not valid UTF-16BE and isn't a legal `bfchar` `<dst>` string per the PDF
/// spec — every codepoint above the BMP produced a malformed ToUnicode
/// entry). Fixed the same way upstream eventually did in the unrelated,
/// API-incompatible 0.12.0 rewrite (`cmap.rs`): encode the destination as
/// UTF-16BE, which is one `{:04x}` unit for BMP codepoints (unchanged
/// output) and a proper two-unit surrogate pair above U+FFFF.
            ...
            match std::char::from_u32(unicode) {
                Some(c) => {
                    ...
                    for unit in c.encode_utf16(&mut utf16_buf) {
                        target_hex.push_str(format!("{:04x}", unit).as_str());
                    }
                }
                None => target_hex.push_str(format!("{:04x}", unicode).as_str()),
            }
            cid_to_unicode_map.push_str(format!("<{:04x}> <{}>\n", glyph_id, target_hex).as_str());
        }
