...
/// `TexCheckFn`. All of these currently accept format `"tex"` when
/// they restrict formats at all (PRE-001..008, OPER-003 restrict to
/// `{"tex","rnw"}`) — see this module's doc comment on why the format
/// gate is deferred.
mod rules_registry {
    use super::*;
...
pub fn run(config: &ToolConfig, document: &ParsedDocument) -> ComplianceReport {
    ...
    for entry in rules_registry::TEX_RULES {
        if config.ignore_rules.contains(entry.id) {
            continue;
        }
        let (meets_floor, confidence) = confidence_meets_floor(entry.id, config.min_confidence);
        if !meets_floor {
        ...
        run_one(entry.id, rule_violations);
    }
