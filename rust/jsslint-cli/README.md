...
A command-line style checker for LaTeX/BibTeX manuscripts submitted to the
...
the JSS-checker distributions — the only one with `--fix`, the `explain` /
`diff` / `init` / `report` subcommands, and an LSP server.
...
jsslint --mode reviewer paper.tex             # per-category PASS/FAIL table
...
A directory argument expands to every `.tex`/`.ltx`/`.bib` file inside it.
A `.jss-lint.toml` in the current directory (or an ancestor) supplies defaults
